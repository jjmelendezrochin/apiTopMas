...
if(isset($postdata) && !empty($postdata))
{
  ...
  // Create.
  $sql = "INSERT INTO `cat_promotor`(`idpromotor`,`idempresa`,`idsupervisor`,`nombre`,`apellidos`,`idusuario`,`rol`,`correo`,`estatus`,`QR`,`uda`,`fda`,`udc`,`fdc`,idestatus,`pwd`) 
  VALUES (null,{$idempresa},{$idsupervisor},'{$nombre}','{$apellidos}','{$idusuario}','{$rol}','{$correo}',{$estatus},'{$QR}','{$uda}','{$fda}','{$udc}','{$fdc}',1,'{$pwd}');";
