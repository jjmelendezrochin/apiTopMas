...
if($cadena){
$condicion = " and (cr.Tienda like '{$cadena}%' or cr.direccioncompleta like '{$cadena}%' "
. "or concat(cc.nombrecorto, ' ', cf.formato, ' ' , cr.Tienda) like '{$cadena}%')  {$condicion_rutas}";
}

//$usuario = [];
$sql = "Select distinct cr.idruta, cr.determinante,   cr.tienda as Tienda, cr.direccioncompleta as Direccion,
...
// cc.idempresa in (Select valor from configuracion where idconf = 2)
// and cf.idempresa in (Select valor from configuracion where idconf = 2)
//echo($sql);
//echo ('<br>');
