...
$sql = "Select distinct cp.idproducto, cp.upc, cp.descripcion, cp.descripcion1,cp.cantidad_caja, cp.cantidad_kgs, 1 as idempresa, cp.categoria1, cp.categoria2, cp.udc, cp.uda,cp.fdc,  
DATE_FORMAT(cp.fdc,'%d/%m/%Y') as fdc1, cp.fda, DATE_FORMAT(cp.fda,'%d/%m/%Y') as fda1,cp.idestatus 
from cat_productos cp
left join cat_empresa ce on cp.idempresa = ce.idempresa where ce.idempresa = (Select cfg.valor from configuracion cfg where cfg.idconf = 2);";
...
if($result = mysqli_query($con,$sql))
{
  ...
  while($row = mysqli_fetch_assoc($result))
  {
    $idproducto = $cat[$i]['idproducto']    =   $row['idproducto'];
