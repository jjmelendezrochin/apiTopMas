...
/* 	$Hora = date("d-m-Y H:i:s");
    $file = fopen("log_" . date("dmY") . ".txt", "a");
	fwrite($file,  '// Obteniendo la version de la App en la base de datos
...
// *****************************
// Fecha hora servidor jjcorp
if ($serverName == "www.jjcorp.com.mx")
{
    $criteriofecha = "cast(DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL -6 hour)  as date)";
}
else
{
    $criteriofecha = "cast(CURRENT_TIMESTAMP() as date)";
}
...
if ($tipo == 0) {
    ...
    '" . $version . "' as version, ccal.fecha
    from cat_rutas cr 
    inner join cat_calendario ccal on cr.idruta  = ccal.idruta  
    left join cat_formato cf on cr.idformato = cf.idformato
    left join cat_cadena cc on cf.idcadena = cc.idcadena
    left join vw_consultastiendaspromotorfecha v on cr.idruta = v.idruta and cr.idruta = v.idruta   
    and v.idpromotor='" . $idpromotor . "'  
    and cast(v.Fecha as date) = " . $criteriofecha . "
    and cast(ccal.Fecha as date) = " . $criteriofecha . "
    where ccal.idpromotor = '" . $idpromotor . "' 
	and cr.idestatus = 1 " . $criterio . " 
    and cast(ccal.fecha as date) >= " . $criteriofecha . "
    and ifnull(v.Cta,0) <2
    order by ccal.fecha asc";
    
    /*$Hora = date("d-m-Y H:i:s");
    $file = fopen("log_" . date("dmY") . ".txt", "a");
	fwrite($file,  '*****************************************************
    // Consulta de tiendas del calendario'. PHP_EOL);
...
if ($tipo == 1) {
    ...
    and cast(v.Fecha as date) = " . $criteriofecha . "
    and cast(ccal.Fecha as date) = " . $criteriofecha . "
    where ccal.idpromotor in (Select distinct idpromotor from promotores_supervisor where idsupervisor = " . $idpromotor . ") 
	and cr.idestatus = 1 " . $criterio . " 
    and cast(ccal.fecha as date) >= " . $criteriofecha . "
    and ifnull(v.Cta,0) <2
    order by ccal.fecha asc";
...
if ($tipo == 1 && $idpromotor == 748) {
    $sql = "Select distinct a.idruta, IFNULL(a.determinante,0) as determinante, a.tienda, 
        a.direccioncompleta, a.latitud, a.longitud, a.version
    From 
    (
    Select distinct
    ...
    concat(IFNULL(cc.nombrecorto,''), ' ', IFNULL(cf.formato,''), ' ' , IFNULL(cr.Tienda,'')) as tienda,
    cr.`direccioncompleta`, cr.latitud, cr.longitud,
    cc.cadena, cf.formato, '" . $version . "' as version 
    from cat_rutas cr    
    left join cat_formato cf on cr.idformato = cf.idformato
    left join cat_cadena cc on cf.idcadena = cc.idcadena
...
$sql11 = "Select idincidencia, descripcion from cat_incidencias order by descripcion;";
if ($result11 = mysqli_query($con, $sql11)) {
    $n = 0;
    while ($row11 = mysqli_fetch_assoc($result11)) {
        $inc['idincidencia'] = $row11['idincidencia'];
        $inc['descripcion'] = $row11['descripcion'];
