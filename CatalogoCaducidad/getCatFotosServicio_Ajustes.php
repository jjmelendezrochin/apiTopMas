...
//******************************
  ...
  (Select concat(cp.descripcion,', ',cp.upc) as producto 
  from caducidad c left join cat_productos cp on c.idproducto = cp.idproducto where c.id = p.id) as producto,
  (Select c.lote as lote 
  from caducidad c left join cat_productos cp on c.idproducto = cp.idproducto where c.id = p.id) as lote,
  (Select date_format(c.caducidad,'%d/%m/%Y') as caducidad 
  from caducidad c left join cat_productos cp on c.idproducto = cp.idproducto where c.id = p.id) as caducidad,
  (Select c.piezas as piezas 
  from caducidad c left join cat_productos cp on c.idproducto = cp.idproducto where c.id = p.id) as piezas,
  ...
  null as latitud_tienda,
  null as longitud_tienda,
  null as latitud_ubicacion,
  null as longitud_ubicacion,
  cr.direccioncompleta as Direccion,
  null AS Distancia_m,
  null AS Distancia,
  (Select DATE_FORMAT(caducidad, '%d/%m/%Y') as fechacaducidad 
	from caducidad c 
	left join cat_productos cp on c.idproducto = cp.idproducto 
	where c.id = p.id
  ) as fechacaducidad
  FROM photos p
...
if($result = mysqli_query($con,$sql))
{
  ...
  while($row = mysqli_fetch_assoc($result))
  {
	...
	$cat[$i]['lote']      =   $row['lote'];
	$cat[$i]['caducidad']      =   $row['caducidad'];
	$cat[$i]['piezas']      =   $row['piezas'];
	$cat[$i]['demo']      =   $row['demo'];
	...
	$cat[$i]['fechacaducidad']              =   $row['fechacaducidad'];
    $i++;
  }
  //$cat[]["sql"] = $sql;
