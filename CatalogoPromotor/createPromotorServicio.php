...
$sql = "Select count(*) as cta from cat_promotor where idusuario = '{$idusuario}'";
...
/*********************************************+*/  
  ...
  if($cta == 0){
  // Create.
  $sql = "INSERT INTO `cat_promotor`(`idpromotor`,`idempresa`,`idsupervisor`,`nombre`,`apellidos`,`idusuario`,`rol`,`correo`,`estatus`,`QR`,`idusohorario`,`uda`,`fda`,`udc`,`fdc`,idestatus,`pwd`) 
  VALUES (null,{$idempresa},{$idsupervisor},'{$nombre}','{$apellidos}','{$idusuario}','{$rol}','{$correo}',{$estatus},'{$QR}','{$idusohorario}','{$uda}','{$fda}','{$udc}','{$fdc}',1,'{$pwd}');";
