...
//$usuario = [];
$sql = "Select distinct p.*,DATE_FORMAT(p.fda,'%d/%m/%Y') as fda1,DATE_FORMAT(p.fdc,'%d/%m/%Y') as fdc1, e.`nombreempresa`,concat(p.`nombre`,' ',p.`apellidos`) as nombrecompleto_s from cat_promotor p left join cat_empresa e on p.idempresa = e.idempresa where p.tipo = 0 and p.idpromotor='{$idpromotor}' order by 1;";
//echo($sql);
