...
//$usuario = [];
$sql = "Select p.*,DATE_FORMAT(p.fda,'%d/%m/%Y') as fda1,DATE_FORMAT(p.fdc,'%d/%m/%Y') as fdc1, e.`nombreempresa`
from cat_promotor p
...
where p.tipo = 0 " . $condicion  . "
and p.idestatus = 1 and p.idempresa = '". $idEmpresa . "' 
group by p.idpromotor " . $orden . ";";
//echo($sql);
