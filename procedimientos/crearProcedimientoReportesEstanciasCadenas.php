...
require '../database.php';
  ...
  // Create.
  $sql_drop = "DROP PROCEDURE IF Exists proc_reporteestanciascadenas;";
  $sql = "CREATE PROCEDURE proc_reporteestanciascadenas
  ( IN 
...
Begin
 ...
 Declare orden varchar(100);
  ...
  Call Proc_Reporte_Todos(@FechaIni, @FechaFin, @idpromotor, @idcadena);
  ...
  from vw_base_reporte vr
  left join vw_tiendas cr
  on vr.idruta = cr.idruta
  left join vw_promotores_supervisores cp
  on vr.idpromotor = cp.idpromotor
  left join vw_vistagrupo_fechapromotortienda vg
  on cr.idruta = vg.idruta and cp.idpromotor = vg.idpromotor and vr.fecha = vg.Fecha
  left join vw_vistagrupo_fechapromotortienda_checkin v1
  on vg.Fecha = v1.Fecha and vg.idpromotor = v1.idpromotor and vg.idruta = v1.idruta
  left join vw_vistagrupo_fechapromotortienda_checkout v2
  on vg.Fecha = v2.Fecha and vg.idpromotor = v2.idpromotor and vg.idruta = v2.idruta
  inner join (
  Select idpromotor from cat_promotor where tipo = 0 and idestatus = 1 and idempresa = (Select cfg.valor from configuracion cfg where cfg.idconf = 2) 
...
";
  ...
  {   
    echo "Procedimiento proc_reporteestanciascadenas eliminado";
  }
  else
  {
    echo $sql;
  ...
  {   
    echo "Procedimiento proc_reporteestanciascadenas creado";
  }
  else
  {
    echo $sql;
